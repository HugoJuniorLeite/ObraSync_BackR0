...
export async function create_rdo_controller(req, res) {
  ...
  try {
    ...
    await Promise.all(
      Object.entries(files).map(async ([key, arr]) => {
        fotosUrls[key] = await upload_photo_to_supabase(arr[0]);
      })
    );

    // const mapped = map_fields_to_english(body, fotosUrls);
    const created = await rdo_service.create_rdo_service(mapped);
...
export async function generate_rdo_pdf_controller(req, res) {
  ...
  try {
    ...
    if (rdoData.street_data.name === "geral_esquerda") {
      ...
      doc.image(page_two, 30, 30, {
        width: doc.page.width / 1.1,
      ...
      if (rdoData.trenches && rdoData.trenches.length > 0) {
        ...
        try {
          ...
          doc.image(logoPath, 20, 20, {
            width: doc.page.width / 1.06,
            height: doc.page.height / 20
          }); // x=50, y=20, largura=100
        } catch (err) {
